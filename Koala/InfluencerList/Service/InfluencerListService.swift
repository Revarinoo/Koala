...
struct InfluencerListService {
    ...
    func getInfluencerList(completionHandler:@escaping(_ result: InfluencerListModel?)->Void) {
        ...
        HttpUtility.shared.request(request as URLRequest, resultType: InfluencerListModel.self) { response in
