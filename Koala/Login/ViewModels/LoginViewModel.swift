...
class LoginViewModel: ObservableObject {
    ...
    func callLoginInfluencer(email: String, password: String) {
        print("jalan")
        var loginInfo: LoginModel = LoginModel()
        loginInfo.email = email
        loginInfo.password = password
        loginInfo.type_role = "Influencer"
        loginService.postBusiness(postRequest: loginInfo) { response in
            if let responseData = response?.token {
                print(responseData)
            }
        }
    }
