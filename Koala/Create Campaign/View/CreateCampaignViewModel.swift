...
class CreateCampaignViewModel : ObservableObject {
    ...
    @Published var createContentModel : [CreateContentModel] = [CreateContentModel(contentType: productType.post.rawValue, contentDetail: "", isDeleted: false)]
...
//        }
    ...
    func submitContent(submittedContent: [CreateContentModel]){
        var createContent : [CreateCampaignDetail] = []
        for content in submittedContent{
            if !content.isDeleted {
                createContent.append(CreateCampaignDetail(content_id: self.content_id, content_type: content.contentType, instruction: content.contentDetail))
            }
